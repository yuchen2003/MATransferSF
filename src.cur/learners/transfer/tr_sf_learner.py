...
class TransferSFLearner:
    def __init__(self, mac, logger, main_args) -> None:
        ...
        self.task2weight = mac.task2weight
    ...
    def _w_solver(self, phi, rewards, task, mask):
        w = self.task2weight[task].detach().clone().requires_grad_()
        ...
        self.task2weight[task] = w.detach().clone()
        return self.task2weight[task]
    ...
    def train(self, batch, t_env: int, episode_num: int, task, is_online=False):
        ...
        # weight = self._w_solver(phi_hat.detach(), rewards, task, expand_mask)
        weight = 1
