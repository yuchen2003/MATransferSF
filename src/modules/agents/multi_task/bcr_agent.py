...
class BCRAgent(nn.Module):
    ...
    def init_hidden(self):
        ...
        return self.decoder.q_skill.weight.new(1, self.args.entity_embed_dim).zero_()

    def forward(self, inputs, hidden_state, task):
        q, h_dec = self.decoder(inputs, hidden_state, task)
        return q, h_dec
...
class Decoder(nn.Module):
    ...
    def __init__(self, task2input_shape_info, task2decomposer, task2n_agents, decomposer, args):
        ...
        self.own_value = nn.Linear(wrapped_obs_own_dim + 1, self.entity_embed_dim)
    ...
    def forward(self, inputs, hidden_state, task):
        ...
        obs_inputs, last_action_inputs, agent_id_inputs = inputs[:, :obs_dim], inputs[:, obs_dim:obs_dim + last_action_shape], inputs[:, obs_dim + last_action_shape:-1]
        return_info = inputs[:, -1:]
        ...
        own_obs = th.cat([own_obs, agent_id_inputs, compact_action_states, return_info], dim=-1)
