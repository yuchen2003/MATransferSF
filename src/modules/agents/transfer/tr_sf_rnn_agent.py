...
class TrSFRNNAgent(nn.Module):
    ...
    def pretrain_forward(self, inputs, cur_action, hidden_state, task):
        ...
        ## decoder />
        wo_action_layer_in = phi_hid.reshape(bsn, -1)
        ...
        phi_hid = phi_hid.view(-1, n_agents, self.phi_dim, self.phi_hidden)
        phi = self.phi_proj(phi_hid).squeeze(-1) # (bs, n, d_phi)

        return h, phi, mu, logvar, action_recon
    ...
    def forward(self, inputs, cur_action, hidden_state, task, task_weight): # psi forward
        ...
        # phi_tilde = self.phi_proj(phi_tilde_hid).squeeze(-1)
        
        # return h, psi, phi_tilde
        return h, psi
    ...
    def _build_policy(self, surrogate_decomposer):
        ...
        self.wo_action_layer_phi = FCNet(self.phi_dim * self.phi_hidden, n_actions_no_attack)
