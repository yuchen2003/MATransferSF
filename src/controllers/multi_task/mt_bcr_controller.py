...
class BCRMAC:
    ...
    def forward(self, ep_batch, t, task, test_mode=False):
        ...
        if not test_mode:
            ...
            return_info = th.flip(th.cumsum(rewards, dim=1), dims=[1])
            return_info = return_info[:, t].repeat(self.task2n_agents[task], 1)
        else:
            return_info = self.return_info - ep_batch["reward"][:, t].item()
    ...
    def init_hidden(self, batch_size, task):
        ...
        self.return_info = th.full((batch_size * n_agents, 1), 20.0, device=hidden_states.device)
    ...
    def _build_inputs(self, batch, t, task):
        ...
        inputs = th.cat([x.reshape(bs * n_agents, -1) for x in inputs] + [self.return_info], dim=1)
        return inputs
